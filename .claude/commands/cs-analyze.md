...
- `python script.py | Out-File -FilePath "output.txt" -Encoding utf8`

❌ 避ける形式（承認が求められます）:
- `python < input.py`（リダイレクト）
- `sh -c "python ..."`（sh ラッピング）
- `cat script.py | python`（パイプ）

**Step 1 実行方法:**

Bash / PowerShell いずれの形式でも実行可能です：

【Bash】
```bash
cd claude-code-inquiry-analyzer && python -c "import sqlite3, pandas as pd, sys; sys.stdout.reconfigure(encoding='utf-8'); ...(Python コード)"
```
...
python -c "import sqlite3, pandas as pd, sys; sys.stdout.reconfigure(encoding='utf-8'); ...(Python コード)"
```
...
サブエージェントがコマンドを実行するときは、以下の形式を **必ず** 使用させてください：

✅ 承認不要（settings.json で許可済み）:

【Bash / Linux / Mac】
```bash
cd claude-code-inquiry-analyzer && python -c "import sqlite3; ..."
python data/churn_weekly.py
sqlite3 data/cs_poc.db "SELECT ..."
```
