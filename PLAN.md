...
- [x] `KPIRow` コンポーネント（問い合わせ総数・対応中・エスカレーション・満足度・高リスク企業数）
- [x] `TrendChart` コンポーネント（Chart.js 積み上げ棒グラフ）
- [x] `ABCTable` コンポーネント（トピック別 ABC ランク）
- [x] `InquiryTable` コンポーネント（フィルタ UI 付き）
- [x] `RiskList` + `ServiceChangeLog` コンポーネント
- [x] `AnalysisModal`（4ステップ進捗表示）
- [x] 期間ボタン（7 / 14 / 28 / 90 日）

### Phase 3: 分析フロー整備（05/14 午後）

- [x] CLAUDE.md の分析手順を詳細化（プロンプト調整）
- [x] テスト実行：「ログインできないトピックを分析して」
- [x] レポート HTML の出力確認・テンプレート調整

### Phase 4: 評価・実測値取得（05/15）

- [x] `evals/eval_questions.md` に標準質問10問を定義
- [x] Claude Code で評価実行・結果記録
- [x] 正答率・分析品質・レビュー工数を PoC 計画書に転記
